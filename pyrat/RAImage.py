...
class GridParams(object):
    """
    A simple container for gridding parameters W (the size of the grid
    convolution function) and alpha (the oversampling ratio).
    ...
    def __init__(self, alpha=1.5, W=6):
        ...
        Args:
            alpha: The oversampling ratio (ratio between the size of the
                oversized image used during gridding to the requested image
                size.)
            W: The width of the GCF in number of pixels (the same for all axes)
...
class Image(object):
    ...
    Basic operations are done by looping over the image, only reading parts
    into memory at a time and storing what is not used on disk.

    Attributes:
        fn: File name string indicating the file in which the image object
            should be stored.
...
#            raise Exception("Invalid number of coordinate axes.")
        ...
        for i in range(len(c)):
            if type(c[i]) is tuple:
                ...
                raise Exception("Image cube coordinate axes must be defined " +
                    "as either a tuple containing (dx, Nx) or an array of " +
                    "values.")
    ...
    def find_max(self, func=None):
        ...
        Args:
            func: A function to apply to the image before looking for the
                maximum.
        ...
        for i in range(self.im.shape[0]):
            ...
            if othertype == IO:
                ...
                if other.shape != rs.shape:
                    raise Exception("Array size does not match size of a " +
                                    "slice in the Data object!")
                ro = other
            #rt = target.im[i, :]
    ...
    def crop_osimage(self):
        ...
        for i in range(self.shape[0]):
            self.im[i] = self.osim.im[i + phindx,
                                      decndx:decndx + self.shape[1],
